...
class HomeAssistantManager @Inject constructor(
    ...
    fun subscribe(
        ...
        Timber.d("开始订阅实体: $entityId")

        // 如果已经存在轮询任务，先检查其状态
        pollingJobs[entityId]?.let { existingJob ->
            if (existingJob.isActive) {
                Timber.d("实体 $entityId 已有活跃的订阅，跳过重复订阅")
                return
            }
        }
                ...
                while (isActive) {
                    try {
                        ...
                        if (state != null) {
                            ...
                            // 只有当状态发生变化时才通知UI
                            if (state != lastState) {
                                Timber.d("实体 $entityId 状态变化: $lastState -> $state")
                            ...
                            // 状态获取失败，增加重试计数
                            retryCount++
                            Timber.w("获取实体 $entityId 状态失败，重试次数: $retryCount")

                            // 如果重试次数超过最大值，发送一个默认状态
                            if (retryCount >= maxRetries) {
                                Timber.e("获取实体 $entityId 状态失败，已达到最大重试次数")
                        ...
                        Timber.e(e, "获取实体状态异常: $entityId")
                        retryCount++

                        // 如果重试次数超过最大值，发送一个默认状态
                        if (retryCount >= maxRetries) {
                            Timber.e("获取实体 $entityId 状态异常，已达到最大重试次数")
                            val defaultState = "unknown"
                            val json = JSONObject().put("state", defaultState)
                            onMessageReceived(json.toString())
                            lastState = defaultState
    ...
    suspend fun fetchEntityState(entityId: String): String? =
            ...
            try {
                ...
                try {
                    ...
                    if (responseCode == 200) {
                        ...
                        val errorStream = connection.errorStream
                        val errorMessage = errorStream?.bufferedReader()?.use { it.readText() } ?: "Unknown error"
                        Timber.e("调用服务失败: HTTP $responseCode - $errorMessage")
                        withContext(Dispatchers.Main) {
